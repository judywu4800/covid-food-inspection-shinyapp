...
icons <- awesomeIcons(
  icon = "ios-close",
  library = "ion",
  markerColor = colorNumeric(palette="YlOrBr", domain= score$score, na.color = 'transparent')
)
...
#interactive map compared by year  
    ...
    output$top100 <- renderLeaflet({
      nc_pal= colorNumeric(palette="YlOrBr", domain= score_year[[1]]$score,na.color = 'transparent')
     leaflet()%>%
  addProviderTiles("CartoDB")%>%
  addCircleMarkers(
